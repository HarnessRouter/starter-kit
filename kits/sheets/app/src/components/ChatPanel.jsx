...
// is reifyui's ChatPanel. What is left here is the only part that is about SHEETS: the two
...
//                a brand-new sheet lives, out of the transcript.
//   loadHistory  the session's turns as messages; a pending sheet has no session, so it answers
//                [] without a request.
...
import { isPending, takeAttachments } from '../lib/sh';
...
export function ChatColumn({ sheetId, seed, title, agentBusy, onSheetMaybeChanged, onSessionStarted,
  ...
  const runTurn = useCallback(({ sessionId, text, attachments, handlers }) => {
    // A file picked on the landing page belongs to this sheet's FIRST message: the panel sends
    // the landing prompt as a seed, which carries no attachments of its own.
  ...
  return (
    <ChatPanel
      ...
      headerRight={agentBusy
        ? <span className="sh-chat-mark" title="The agent is working"><Sparkles size={12} /></span>
        : null}
      collapsed={collapsed}
      ...
      busyState={(
        <div className="uic-chat-empty">
          ...
          <div className="uic-chat-empty-t">Building your sheet…</div>
      ...
      emptyState={(
        <div className="uic-chat-empty">
          <Sparkles size={26} />
          <div className="uic-chat-empty-t">Your sheet copilot</div>
