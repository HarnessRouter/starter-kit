...
//   #/s/{id}      the sheet (?seed=<first copilot message>, ?tpl=<template it came from>)
//
...
function parseHash() {
  ...
  if (path.startsWith('s/')) {
    return { page: 'sheet', id: decodeURIComponent(path.slice(2)),
...
export default function App() {
  ...
  return (
    <DialogHost>
      {route.page === 'sheet'
        ? <SheetPage key={route.id} id={route.id} seed={route.seed} template={route.template} />
        : <LandingPage />}
    </DialogHost>
  );
}
