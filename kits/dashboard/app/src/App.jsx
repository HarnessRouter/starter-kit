// Hash router.
//   #/            landing (prompt, templates, my dashboards)
//   #/d/{id}      the dashboard (?seed=<first copilot message>, consumed once on arrival)
//
...
function parseHash() {
  ...
  if (path.startsWith('d/')) {
    // The template a NEW dashboard starts from rides in the id itself ("new:<template>"), so
    // there is no second place for it to be carried and no way for the two to disagree.
    return { page: 'dashboard', id: decodeURIComponent(path.slice(2)), seed: params.get('seed') || '' };
  }
  return { page: 'landing' };
}
  ...
  return (
    <DialogHost>
      {route.page === 'dashboard'
        ? <DashboardPage key={route.id} id={route.id} seed={route.seed} />
        : <LandingPage />}
    </DialogHost>
  );
}
